% This script computes the statistics over for each measurement site
...
for ff = 1:length(frequencies)
    ...
    p2 = [];
        ...
        file_total = ["Results_UK_P452_" locations{loc}  "_" num2str(pdr_type) ".xls"];
        filename_total = strjoin(file_total, '');

        file_total_freq = ["Results_UK_P452_" locations{loc} "_" frequencies{ff} "_" num2str(pdr_type) ".xls"];
        ...
        if ~exist(filename_out,'file')
            errormsg = strjoin( [filename_out " does not exist. Stopping."], '');
            error(errormsg);
        end
